...
const useSyncEditableText = <T extends HTMLElement>(
...
const useSyncEditableHtml = <T extends HTMLElement>(
  ref: React.RefObject<T | null>,
  value: string,
  enabled = true
) => {
...
export default function EditPageClient() {
  ...
  const planColumnOptions = [
    ...
    readonly [keyof typeof planColumns, string, boolean]
  >;
  const planStageRefs = useRef<Record<string, HTMLTextAreaElement | null>>({});
  ...
  const applyTemplate = async (block?: typeof activeBlock) => {
    ...
    if (!proposal) return;
    if (!block) {
      const emptyProposal: Proposal = { ...defaultValues };
      (Object.keys(emptyProposal) as Array<keyof Proposal>).forEach((key) => {
        if (typeof emptyProposal[key] === "string") {
          (emptyProposal as Record<keyof Proposal, unknown>)[key] = "";
        }
      });
      setProposal({
        ...emptyProposal,
  ...
  const controlMap: Record<ProposalField, { history: boolean }> = {
    ...
    casesCommentVisible: { history: false },
  };
  const showHistory = activeField
    ? controlMap[activeField]?.history
    : true;

  const clientRef = useEditable<HTMLSpanElement>(proposal.clientName);
  const serviceRef = useEditable<HTMLSpanElement>(proposal.serviceName);
              ...
              <div className="grid grid-cols-1 gap-2 md:grid-cols-5 md:items-end">
                <div className="flex flex-col gap-1 md:col-span-1 md:justify-end md:self-end">
                  <div className="proposal-headline text-[10px] uppercase tracking-[0.2em] text-zinc-400">
                    ...
                    <br />
                    <span
                      ref={useEditable<HTMLSpanElement>(proposal.validUntil ?? "")}
                ...
                <div className="flex flex-col md:col-span-1 md:col-start-5 md:justify-end md:self-end">
                  <div
                    ref={useEditable<HTMLDivElement>(proposal.contactPhone ?? "")}
                  ...
                  />
                  <a
                    ref={useEditable<HTMLAnchorElement>(proposal.contactEmail ?? "")}
                  ...
                  <div className="mt-[2px] flex items-center gap-1 text-[15px] text-zinc-700 leading-[1]">
                    <span>tg</span>
                    <a
                      ref={useEditable<HTMLAnchorElement>(proposal.contactTelegram ?? "")}
            ...
            {activeBlock === "plan" && (
              <section className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
                ...
                <div className="mt-3 flex flex-col gap-2 text-xs text-zinc-700">
                  {planColumnOptions.map(([key, label, locked]) => (
                    <label key={key} className="flex items-center gap-2">
                      {!locked && (
                        <input
                          type="checkbox"
                          checked={planColumns[key]}
                          onChange={(event) =>
                            setPlanColumns((prev) => ({
                              ...prev,
